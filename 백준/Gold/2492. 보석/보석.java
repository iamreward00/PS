...
// T 위치놈들만 돌려본건데 이거 아닌거같은데 3,5 같은 경우가 -> 현재위치가 침입자가아닌경우 정답일수도있을꺼같은데 최적화방법이 떠오르지않아요...
    for (int[] p1 : trr) { /// 와 그래서 x 좌표 전체 y 좌표 전체야?????????????????????와 미쳤다
        ...
        for (int[] p2 : trr) {
            ...
            for (int[] target : trr) {
            if (target[1] >= startX && target[1] <= endX &&
                target[0] >= startY && target[0] <= endY) {
                currentCount++;
            }
        }
            if (max < currentCount) {
            max = currentCount;
            // 최종 출력할 사각형의 왼쪽 '아래' 좌표를 저장
            // 단, 경계를 벗어나지 않도록 조정
            midx = Math.min(startX, N - K);
            mjdx = Math.min(startY, M - K);
        }
        }
    }		
